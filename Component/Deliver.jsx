import React, { useState } from "react";
import { FaArrowCircleLeft } from "react-icons/fa";
import { FaArrowCircleRight } from "react-icons/fa";
import Testimonials from "./Testimonial";




const Deliver = () => {
  const cards = [
    {
      title: "Breakfast Paratha",
      image:
        "https://sehatful.s3.ap-south-1.amazonaws.com/sehatfull-assets/Category/fWCj0qM43C-1745310728347.png",
      link: "#paratha",
    },
    {
      title: "Daily Fresh Juice",
      image:
        "https://sehatful.s3.ap-south-1.amazonaws.com/sehatfull-assets/Category/Wx6H1BfY1e-1745310483790.png",
      link: "#juice",
    },
    {
      title: "Green Salad",
      image:
        "https://sehatful.s3.ap-south-1.amazonaws.com/sehatfull-assets/Category/Xyzt0gX23a-1745310449990.png",
      link: "#salad",
    },
    {
      title: "Fruit Bowl",
      image:
        "https://sehatful.s3.ap-south-1.amazonaws.com/sehatfull-assets/Category/Zr4Fd0wPaA-1745310401234.png",
      link: "#fruit",
    },
  ];

  const [startIndex, setStartIndex] = useState(0);

  const showNext = () => {
    setStartIndex((prev) => (prev + 2) % cards.length);
  };

  const showPrev = () => {
    setStartIndex((prev) =>
      (prev - 2 + cards.length) % cards.length
    );
  };

  const visibleCards = [
    cards[startIndex],
    cards[(startIndex + 1) % cards.length],
  ];

  return (
    <div className="min-h-screen bg-green-50 relative font-sans px-4">
      {/* Header */}
      <div className="items-center mb-6 flex justify-between">
        <div class="flex gap-6 items-center">
    <div class="h-[60px] sm:h-full">
      <img 
        alt="leaf" 
        class="h-full" 
        src="data:image/png;base64," 
      />
    </div>
    <div class="flex flex-col">
      <span class="text-[#4d6042] text-4xl md:text-4xl font-bold title">
        Deliver Now
      </span>
      <span class="text-[#4d6042] text-base md:text-2xl">
        Get instant delivery for these categories
      </span>
    </div>
  </div>
        <div className="space-x-3 flex right-0">
          <button
            className=" bg-[#4d6042] text-white rounded-full flex items-center justify-center cursor-pointer hover:shadow-lg"
            onClick={() => scroll('left')}
          >
            <FaArrowCircleLeft className="w-10 h-10"/>
          </button>
          <button
            className=" bg-[#4d6042] text-white rounded-full flex items-center justify-center cursor-pointer hover:shadow-lg"
            onClick={() => scroll('right')}
          >
            <FaArrowCircleRight className="w-10 h-10"/>
          </button>

        </div>

      
      </div>
      <div className="flex flex-wrap gap-6 p-6 bg-[#FAFCF8]">
  {/* Breakfast Paratha Card */}
  <div className="flex items-center justify-between bg-white rounded-3xl shadow-md p-6 w-full max-w-md hover:scale-105">
    <div className="flex items-center gap-4">
      <img
        src="https://img.freepik.com/premium-photo/indian-traditional-breakfast-food-stuffed-paratha-served-with-curd-butter_466689-77273.jpg" 
        alt="Breakfast Paratha"
        className="w-24 h-24 object-cover rounded-full"
      />
      <h2 className="text-lg font-medium text-gray-700">Breakfast Paratha</h2>
    </div>
    <button className="bg-[#4C5C3B] cursor-pointer text-white px-4 py-2 rounded-full font-semibold hover:bg-[#3b472d] transition flex items-center gap-1">
      Explore <span className="text-xl">›</span>
    </button>
  </div>

  {/* Daily Fresh Juice Card */}
  <div className="flex items-center justify-between bg-white rounded-3xl shadow-md p-6 w-full max-w-md hover:scale-105 transition-all">
    <div className="flex items-center gap-4">
      <img
        src="https://img.freepik.com/free-vector/tequila-sunrise-cocktail-realistic_1284-3886.jpg?ga=GA1.1.1170109027.1746179818&semt=ais_items_boosted&w=740"
        alt="Daily Fresh Juice"
        className="w-24 h-24 object-cover rounded-full"
      />
      <h2 className="text-lg font-medium text-gray-700">Daily Fresh Juice</h2>
    </div>
    <button className="bg-[#4C5C3B] cursor-pointer text-white px-4 py-2 rounded-full font-semibold hover:bg-[#3b472d] transition flex items-center gap-1">
      Explore <span className="text-xl">›</span>
    </button>
  </div>
</div>
 
 <Testimonials/>
    </div>
  )}
  export default Deliver;